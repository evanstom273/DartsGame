...
func _draw_arena_gradient() -> void:
	for band in range(GRADIENT_BANDS):
		...
		var y: float = amount * SCREEN_SIZE.y
		var height: float = SCREEN_SIZE.y / float(GRADIENT_BANDS) + 1.0
		...
		draw_rect(Rect2(Vector2(0.0, y), Vector2(SCREEN_SIZE.x, height)), color, true)

	draw_rect(Rect2(Vector2(0.0, 0.0), Vector2(divider_x, SCREEN_SIZE.y)), Color(0.0, 0.0, 0.0, 0.10), true)
	draw_rect(Rect2(Vector2(divider_x, 0.0), Vector2(SCREEN_SIZE.x - divider_x, SCREEN_SIZE.y)), _with_alpha(accent_color_primary, 0.020), true)
...
func _draw_far_led_ribbons(phase: float) -> void:
	for row in range(4):
		...
		draw_rect(Rect2(Vector2(0.0, y), Vector2(SCREEN_SIZE.x, panel_height)), _with_alpha(accent_color_primary, row_alpha), true)
...
func _draw_stage_truss() -> void:
	...
	draw_line(Vector2(0.0, 168.0), Vector2(SCREEN_SIZE.x, 96.0), dark_truss_color, 3.0, true)
	draw_line(Vector2(0.0, 174.0), Vector2(SCREEN_SIZE.x, 102.0), truss_color, 1.0, true)
	draw_line(Vector2(0.0, 706.0), Vector2(SCREEN_SIZE.x, 758.0), Color(0.0, 0.0, 0.0, 0.17), 3.0, true)
	draw_line(Vector2(0.0, 712.0), Vector2(SCREEN_SIZE.x, 764.0), _with_alpha(accent_color_primary, 0.050), 1.0, true)
	...
	for index in range(10):
		...
		draw_line(Vector2(x, 0.0), Vector2(x - 210.0, SCREEN_SIZE.y), Color(0.0, 0.0, 0.0, 0.055), 1.0, true)
...
func _draw_light_beams(phase: float) -> void:
	...
	for index in range(3):
		...
		points.append(Vector2(x + 250.0, SCREEN_SIZE.y))
		points.append(Vector2(x + 90.0, SCREEN_SIZE.y))
		draw_colored_polygon(points, _with_alpha(accent_color_secondary, 0.014))


func _draw_crowd_silhouettes(phase: float) -> void:
	draw_rect(Rect2(Vector2(0.0, 790.0), Vector2(SCREEN_SIZE.x, 290.0)), Color(0.0, 0.0, 0.0, 0.20), true)
	...
	for index in range(14):
		...
		draw_rect(Rect2(Vector2(SCREEN_SIZE.x - 120.0, side_y + 12.0), Vector2(120.0, 18.0)), Color(0.0, 0.0, 0.0, right_alpha), true)
...
func _draw_camera_split() -> void:
	draw_rect(Rect2(Vector2(divider_x - 56.0, 0.0), Vector2(56.0, SCREEN_SIZE.y)), Color(0.0, 0.0, 0.0, 0.16), true)
	draw_rect(Rect2(Vector2(divider_x, 0.0), Vector2(78.0, SCREEN_SIZE.y)), _with_alpha(accent_color_primary, 0.040), true)
	draw_rect(Rect2(Vector2(divider_x - 1.0, 0.0), Vector2(2.0, SCREEN_SIZE.y)), _with_alpha(Color(0.80, 0.84, 1.0, 1.0), 0.22), true)
	draw_rect(Rect2(Vector2(divider_x + 2.0, 0.0), Vector2(18.0, SCREEN_SIZE.y)), _with_alpha(accent_color_secondary, 0.030), true)


func _draw_vignette() -> void:
	draw_rect(Rect2(Vector2.ZERO, Vector2(SCREEN_SIZE.x, 128.0)), Color(0.0, 0.0, 0.0, 0.30), true)
	draw_rect(Rect2(Vector2(0.0, SCREEN_SIZE.y - 170.0), Vector2(SCREEN_SIZE.x, 170.0)), Color(0.0, 0.0, 0.0, 0.34), true)
	draw_rect(Rect2(Vector2.ZERO, Vector2(150.0, SCREEN_SIZE.y)), Color(0.0, 0.0, 0.0, 0.24), true)
	draw_rect(Rect2(Vector2(SCREEN_SIZE.x - 155.0, 0.0), Vector2(155.0, SCREEN_SIZE.y)), Color(0.0, 0.0, 0.0, 0.22), true)
