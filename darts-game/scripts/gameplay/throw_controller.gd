...
signal match_complete(result: Dictionary)
...
const FLAG_PATHS: Array[String] = [
	"res://assets/flags/england.svg",
	"res://assets/flags/scotland.svg",
	"res://assets/flags/wales.svg",
	"res://assets/flags/n_ireland.svg",
	"res://assets/flags/ireland.svg",
	"res://assets/flags/netherlands.svg",
	"res://assets/flags/germany.svg",
	"res://assets/flags/austria.svg",
	"res://assets/flags/oceania.svg",
	"res://assets/flags/us.svg",
	"res://assets/flags/canada.svg",
	"res://assets/flags/mexico.svg",
	"res://assets/flags/japan.svg",
	"res://assets/flags/belgium.svg",
	"res://assets/flags/france.svg",
	"res://assets/flags/italy.svg",
	"res://assets/flags/portugal.svg",
	"res://assets/flags/poland.svg",
	"res://assets/flags/spain.svg",
	"res://assets/flags/switzerland.svg",
	"res://assets/flags/czech_republic.svg",
	"res://assets/flags/croatia.svg",
	"res://assets/flags/slovenia.svg",
	"res://assets/flags/latvia.svg",
	"res://assets/flags/lithuania.svg",
	"res://assets/flags/sweden.svg",
	"res://assets/flags/denmark.svg",
	"res://assets/flags/finland.svg",
	"res://assets/flags/norway.svg",
	"res://assets/flags/singapore.svg",
	"res://assets/flags/china.svg",
	"res://assets/flags/philippines.svg"
]
...
const MIN_CONFIRM_INTERVAL_MSEC: int = 220
const TOUCH_MOUSE_SUPPRESS_MSEC: int = 900
...
## Visible line wobble amplitude. Difficulty comes from reading the moving lines, not hidden dart drift.
...
## Player display name label in the compact scoreboard.
@export var player_name_label_path: NodePath = NodePath("../ScoreboardLayer/MatchScoreboard/PlayerRow/Name")
## AI display name label in the compact scoreboard.
@export var ai_name_label_path: NodePath = NodePath("../ScoreboardLayer/MatchScoreboard/AIRow/Name")
## Player flag TextureRect shown before the player name.
@export var player_flag_path: NodePath = NodePath("../ScoreboardLayer/MatchScoreboard/PlayerRow/Flag")
## AI flag TextureRect shown before the AI name.
@export var ai_flag_path: NodePath = NodePath("../ScoreboardLayer/MatchScoreboard/AIRow/Flag")
...
## Current-leg player name label.
@export var player_current_leg_name_label_path: NodePath = NodePath("../ScoreboardLayer/StatsPanel/CurrentLeg/PlayerLabel")
## Current-leg AI name label.
@export var ai_current_leg_name_label_path: NodePath = NodePath("../ScoreboardLayer/StatsPanel/CurrentLeg/AILabel")
## Match-stats player column header.
@export var player_stats_header_label_path: NodePath = NodePath("../ScoreboardLayer/StatsPanel/MatchStats/Header/PlayerColumn")
## Match-stats AI column header.
@export var ai_stats_header_label_path: NodePath = NodePath("../ScoreboardLayer/StatsPanel/MatchStats/Header/AIColumn")
...
var _last_drawn_y: float = 0.0
var _last_drawn_x: float = 0.0
...
var _human_player_controls_enabled: bool = true
var _show_completion_summary: bool = true
var _waiting_for_summary_continue: bool = false
var _ai_darts_thrown: int = 0
var _ai_dart_sequence_index: int = 0
var _ai_profile: CharacterResource
var _player_profile: CharacterResource
var _generated_roster: Array[CharacterResource] = []
var _ai_visit_target_total: int = 60
var _ai_visit_target_remaining: int = 60
var _ai_maximum_attempt: bool = false
var _last_touch_confirm_msec: int = -1000
var _last_confirm_msec: int = -1000
var _suppress_mouse_confirm_until_msec: int = -1000
...
func _ready() -> void:
	...
	_generated_roster = CharacterRoster.generate_default_roster()
	_assign_generated_match_characters()
	_apply_board_scale()
	reset_match()
	set_process(true)
	queue_redraw()
...
func _input(event: InputEvent) -> void:
	if not _human_player_controls_enabled:
		return
	...
	if event is InputEventScreenTouch:
		...
		if touch_event.pressed and _is_throw_input_blocked(touch_event.position):
			return

		if touch_event.pressed and _can_confirm_player_lock() and _can_accept_confirm(now_msec):
			_last_touch_confirm_msec = now_msec
			_suppress_mouse_confirm_until_msec = now_msec + TOUCH_MOUSE_SUPPRESS_MSEC
			_last_confirm_msec = now_msec
			_confirm_lock()
		...
		if mouse_event.pressed and _is_throw_input_blocked(mouse_event.position):
			return

		if _is_mobile_runtime():
			return

		if now_msec < _suppress_mouse_confirm_until_msec or now_msec - _last_touch_confirm_msec < TOUCH_MOUSE_SUPPRESS_MSEC:
			return

		if mouse_event.button_index == MOUSE_BUTTON_LEFT and mouse_event.pressed and _can_confirm_player_lock() and _can_accept_confirm(now_msec):
			_last_confirm_msec = now_msec
			_confirm_lock()
			get_viewport().set_input_as_handled()
		...
		if key_event.pressed and not key_event.echo:
			var now_msec: int = Time.get_ticks_msec()
			if _match_input_enabled and not _match_finished and _active_thrower == THROWER_PLAYER and key_event.keycode == KEY_SPACE and _can_accept_confirm(now_msec):
				_last_confirm_msec = now_msec
				_confirm_lock()
				get_viewport().set_input_as_handled()
...
func _can_confirm_player_lock() -> bool:
	return _match_input_enabled and not _match_finished and _active_thrower == THROWER_PLAYER


func _can_accept_confirm(now_msec: int) -> bool:
	return now_msec - _last_confirm_msec >= MIN_CONFIRM_INTERVAL_MSEC
...
func _is_throw_input_blocked(screen_position: Vector2) -> bool:
	for blocker in get_tree().get_nodes_in_group("throw_input_blocker"):
		if blocker != self and blocker.has_method("blocks_throw_input") and bool(blocker.call("blocks_throw_input", screen_position)):
			return true
...
func hold_for_setup() -> void:
	...
	_human_player_controls_enabled = true
	_show_completion_summary = true
...
func configure_match(selected_starting_score: int, selected_match_format: int, selected_best_of_count: int, selected_starting_thrower: int, selected_double_in_enabled: bool) -> void:
	...
	configure_match_with_profiles(
		_player_profile,
		_ai_profile,
		selected_starting_score,
		selected_match_format,
		selected_best_of_count,
		selected_starting_thrower,
		selected_double_in_enabled,
		true,
		true,
		{}
	)
...
func configure_match_with_profiles(player_profile: CharacterResource, ai_profile: CharacterResource, selected_starting_score: int, selected_match_format: int, selected_best_of_count: int, selected_starting_thrower: int, selected_double_in_enabled: bool, human_controls_enabled: bool = true, show_completion_summary: bool = true, context: Dictionary = {}) -> void:
	...
	_player_profile = player_profile if player_profile != null else CharacterResource.new()
	_ai_profile = ai_profile if ai_profile != null else CharacterResource.new()
	_human_player_controls_enabled = human_controls_enabled
	_show_completion_summary = show_completion_summary
	_match_context = context.duplicate(false)
...
func _assign_generated_match_characters() -> void:
	if _generated_roster.is_empty():
		_generated_roster = CharacterRoster.generate_default_roster()

	_player_profile = CharacterRoster.random_character(_generated_roster)
	_ai_profile = CharacterRoster.random_character(_generated_roster)

	if _generated_roster.size() > 1 and _ai_profile == _player_profile:
		_ai_profile = CharacterRoster.random_character(_generated_roster)

	_apply_player_profile(_player_profile)


func _apply_player_profile(profile: CharacterResource) -> void:
	vertical_speed = profile.vertical_speed
	horizontal_speed = profile.horizontal_speed
	visible_wobble_pixels = profile.visible_wobble_pixels
	control_skill = profile.control_skill
	pressure = profile.pressure
	post_lock_drift_duration = profile.post_lock_drift_duration
	post_lock_drift_pixels = profile.post_lock_drift_pixels
	fatigue_per_dart = profile.fatigue_per_dart
	line_chaos_strength = profile.line_chaos_strength
...
func _begin_player_visit() -> void:
	...
	if _human_player_controls_enabled:
		...
		_ai_visit_target_total = _sample_ai_visit_target_total(THROWER_PLAYER)
		_ai_visit_target_remaining = _ai_visit_target_total
		_ai_maximum_attempt = _should_ai_attack_maximum(THROWER_PLAYER)
...
func _begin_ai_visit() -> void:
	...
	_ai_darts_thrown = 0
	_ai_visit_finished = false
	_ai_visit_target_total = _sample_ai_visit_target_total(THROWER_AI)
	_ai_visit_target_remaining = _ai_visit_target_total
	_ai_maximum_attempt = _should_ai_attack_maximum(THROWER_AI)
...
func _complete_match(winner: int) -> void:
	...
	match_complete.emit(_match_result_payload(winner))


func _match_result_payload(winner: int) -> Dictionary:
	return {
		"winner": winner,
		"player_profile": _player_profile,
		"ai_profile": _ai_profile,
		"player_legs": _player_legs,
		"ai_legs": _ai_legs,
		"player_sets": _player_sets,
		"ai_sets": _ai_sets,
		"summaries": _match_leg_summaries.duplicate(true),
		"context": _match_context.duplicate(false)
	}
...
func _show_stats_summary(title: String, summaries: Array[Dictionary], continue_match: bool) -> void:
	...
	if modal.has_method("show_summary_data"):
		modal.call("show_summary_data", title, _summary_meta_text(summaries), _summary_stats_rows(summaries), _summary_breakdown_rows(summaries), continue_match, _thrower_name(THROWER_PLAYER), _thrower_name(THROWER_AI))
	else:
...
func _summary_stats_text(summaries: Array[Dictionary]) -> String:
	...
	for row_value in _summary_stats_rows(summaries):
		...
		lines.append("%s: %s %s | %s %s" % [str(row["label"]), _thrower_name(THROWER_PLAYER), str(row["player"]), _thrower_name(THROWER_AI), str(row["ai"])])
...
func _summary_meta_text(summaries: Array[Dictionary]) -> String:
	...
	return "%d start | %s | Double out | %s | Legs: %s %d - %d %s" % [starting_score, entry_rule, format_text, _thrower_name(THROWER_PLAYER), player_legs_won, ai_legs_won, _thrower_name(THROWER_AI)]
...
func _summary_breakdown_rows(summaries: Array[Dictionary]) -> Array[Dictionary]:
	...
	for summary in summaries:
		...
		rows.append({
			"leg": leg_title,
			"thrower": _thrower_name(THROWER_PLAYER),
		...
		rows.append({
			"leg": "",
			"thrower": _thrower_name(THROWER_AI),
...
func _summary_breakdown_text(summaries: Array[Dictionary]) -> String:
	...
	for summary in summaries:
		...
		lines.append("%s: %s | darts %d | avg %s | checkout %s" % [
			_thrower_name(THROWER_PLAYER),
...
func _thrower_name(thrower: int) -> String:
	var profile: CharacterResource = _player_profile if thrower == THROWER_PLAYER else _ai_profile

	if thrower == THROWER_PLAYER and _human_player_controls_enabled:
		return "PLAYER (%s)" % _profile_level_name(profile)
...
func _profile_level_name(profile: CharacterResource) -> String:
	if profile == null:
		return "Amateur"
...
func _confirm_lock() -> void:
	if _phase == THROW_PHASE_LOCK_HEIGHT:
		_locked_y = _visible_lock_y()
		_phase = THROW_PHASE_LOCK_WIDTH
		_phase_time = 0.0
	elif _phase == THROW_PHASE_LOCK_WIDTH:
		_locked_x = _visible_lock_x()
		if post_lock_drift_pixels <= 0.0:
...
func _is_pointer_in_throw_area(pointer_position: Vector2) -> bool:
	var board_position: Vector2 = _overlay_position_to_board_local(pointer_position)
	var touch_margin: float = 28.0 / _board_visual_scale()

	return board_position.length() <= AIM_RADIUS + touch_margin


func _is_touch_in_throw_area(screen_position: Vector2) -> bool:
	if _is_pointer_in_throw_area(to_local(screen_position)):
		return true

	var canvas_position: Vector2 = get_global_transform_with_canvas().affine_inverse() * screen_position

	return _is_pointer_in_throw_area(canvas_position)
...
func _process_post_lock_drift(delta: float) -> void:
	...
	if amount >= 1.0:
		_land_player_dart(_drifted_lock_position)
...
func _throw_ai_dart() -> void:
	var thrower: int = _active_thrower
	var target_label: String = _next_ai_target_label(thrower)
	var target_is_checkout: bool = target_label == _finish_label_for_score(_score_for_thrower(thrower))
	var landed_label: String = _ai_landed_label_for_target(target_label, thrower)
	...
	if target_is_checkout and thrower == THROWER_AI:
		_ai_visit_threw_at_checkout = true

	var is_bust: bool = _apply_result_to_score(thrower, result)

	_results.append(result)
	_visit_total += int(result["counted_score"])
	dart_landed.emit(result)
	_record_dart_progress(thrower, result, is_bust)
	_ai_visit_target_remaining = maxi(0, _ai_visit_target_remaining - int(result["score"]))
	_ai_darts_thrown += 1

	if is_bust:
		_visit_bust = true
	elif _score_for_thrower(thrower) == 0:
		_visit_checkout = true
		_leg_finished = true
		_leg_winner = thrower

	if is_bust or _score_for_thrower(thrower) == 0 or _ai_darts_thrown >= darts_per_visit:
		_ai_visit_finished = true
		_ai_wait_time = 0.0
		_finalize_visit(thrower)

	_update_all_live_labels()


func _finish_computer_visit() -> void:
	if _leg_finished:
		_complete_leg(_leg_winner)
		return

	if _active_thrower == THROWER_PLAYER:
		_begin_ai_visit()
	else:
		_begin_player_visit()
...
func _set_static_scoreboard_labels() -> void:
	...
	_update_competitor_name_labels()
...
func _update_competitor_name_labels() -> void:
	var player_name: String = _thrower_name(THROWER_PLAYER)
	var ai_name: String = _thrower_name(THROWER_AI)

	_set_label_text(player_name_label_path, player_name)
	_set_label_text(ai_name_label_path, ai_name)
	_set_label_text(player_current_leg_name_label_path, player_name)
	_set_label_text(ai_current_leg_name_label_path, ai_name)
	_set_label_text(player_stats_header_label_path, player_name)
	_set_label_text(ai_stats_header_label_path, ai_name)
	_set_flag_texture(player_flag_path, _player_profile)
	_set_flag_texture(ai_flag_path, _ai_profile)


func _set_flag_texture(path: NodePath, profile: CharacterResource) -> void:
	var flag: TextureRect = get_node_or_null(path) as TextureRect

	if flag == null:
		return

	flag.texture = _flag_texture_for_profile(profile)
	flag.visible = flag.texture != null


func _flag_texture_for_profile(profile: CharacterResource) -> Texture2D:
	if profile == null:
		return null

	var nation_index: int = int(profile.home_nation)

	if nation_index < 0 or nation_index >= FLAG_PATHS.size():
		return null

	return load(FLAG_PATHS[nation_index]) as Texture2D
...
func _update_checkout_route() -> void:
	...
	if show_ai_checkout_route and _ai_is_in:
		ai_route = _preferred_ai_checkout_route(_ai_score, _darts_remaining_for_route(THROWER_AI))
...
func _next_ai_target_label(thrower: int = THROWER_AI) -> String:
	var score: int = _score_for_thrower(thrower)

	if double_in_enabled and not _is_thrower_in(thrower):
		return _preferred_ai_double_for_score(40)

	var nine_darter_target: String = _ai_nine_darter_target(thrower)

	if nine_darter_target != "":
		return nine_darter_target

	var route: Array[String] = _preferred_ai_checkout_route(score, maxi(0, darts_per_visit - _ai_darts_thrown), _profile_for_thrower(thrower))

	if route.size() > 0:
		return route[0]

	if score <= 170:
		var setup_target: String = _ai_setup_target_for_checkout(thrower)

		if setup_target != "":
			return setup_target

	return _ai_scoring_target_for_visit(thrower)


func _sample_ai_visit_target_total(thrower: int = THROWER_AI) -> int:
	var profile: CharacterResource = _profile_for_thrower(thrower)
	var is_opening_phase: bool = _leg_darts_for_thrower(thrower) < 9 and _score_for_thrower(thrower) > 230
	...
	var pressure_drag: float = _ai_pressure_level(thrower) * (1.0 - profile.nerve) * 18.0
	var opening_push: float = randf_range(6.0, 22.0) * profile.opening_phase_bias if is_opening_phase and profile.average_max >= 90.0 else 0.0
	var heavy_scoring_push: float = randf_range(0.0, 14.0) * maxf(0.0, profile.heavy_scoring_bias - 1.0) if _score_for_thrower(thrower) > 230 else 0.0
	var hot_streak_bonus: float = randf_range(12.0, 34.0) if profile.average_max >= 100.0 and randf() < _ai_hot_streak_chance(is_opening_phase, profile) else 0.0
	var cold_visit_drag: float = randf_range(10.0, 28.0) if profile.average_max >= 90.0 and randf() < _ai_cold_visit_chance(is_opening_phase, profile) else 0.0
	var target_total: int = int(round(average + swing + opening_push + heavy_scoring_push + hot_streak_bonus - cold_visit_drag - pressure_drag))

	return clampi(target_total, 0, 180)


func _ai_hot_streak_chance(is_opening_phase: bool, profile: CharacterResource = null) -> float:
	if profile == null:
		profile = _profile()

	var appetite: float = clampf(profile.maximum_appetite, 0.0, 2.0)

	if profile.average_max >= 100.0:
		return (0.18 if is_opening_phase else 0.10) * appetite

	if profile.average_max >= 90.0:
		return (0.08 if is_opening_phase else 0.04) * appetite

	return 0.0


func _ai_cold_visit_chance(is_opening_phase: bool, profile: CharacterResource = null) -> float:
	if profile == null:
		profile = _profile()

	var scoring_bias: float = clampf(profile.heavy_scoring_bias, 0.0, 2.0)

	if profile.average_max >= 100.0:
		return (0.06 if is_opening_phase else 0.14) * (1.15 - scoring_bias * 0.25)

	if profile.average_max >= 90.0:
		return (0.08 if is_opening_phase else 0.16) * (1.15 - scoring_bias * 0.25)

	return 0.0


func _ai_scoring_target_for_visit(thrower: int = THROWER_AI) -> String:
	...
	for target in profile.preferred_scoring_targets:
	...
	treble_threshold -= clampf(profile.heavy_scoring_bias - 1.0, -1.0, 1.0) * 4.0
...
func _ai_nine_darter_target(thrower: int = THROWER_AI) -> String:
	var score: int = _score_for_thrower(thrower)
	var leg_darts: int = _leg_darts_for_thrower(thrower)
	var profile: CharacterResource = _profile_for_thrower(thrower)

	if score <= 0 or leg_darts >= 9 or _ai_darts_thrown >= darts_per_visit:
		return ""

	if leg_darts < 3:
		return ""

	if leg_darts < 6 and _turn_start_score == starting_score - 180:
		if randf() > clampf(profile.nine_darter_appetite, 0.0, 2.0):
			return ""

		return "T20"

	if leg_darts >= 6:
		var darts_remaining: int = 9 - (leg_darts + _ai_darts_thrown)

		if darts_remaining < 1:
			return ""

		var route: Array[String] = _preferred_nine_finish_route(score, thrower)
...
func _preferred_nine_finish_route(score: int, thrower: int = THROWER_AI) -> Array[String]:
	var preferred_route: Array[String] = _preferred_ai_checkout_route(score, maxi(1, 9 - _leg_darts_for_thrower(thrower)), _profile_for_thrower(thrower))

	if preferred_route.size() > 0:
	...
	match score:
		...
		_:
			return _checkout_route_for_score(score, maxi(1, 9 - _leg_darts_for_thrower(thrower)))


func _preferred_ai_checkout_route(score: int, max_darts: int = 3, profile: CharacterResource = null) -> Array[String]:
	var route: Array[String] = _exact_checkout_route_from_profile(score, max_darts, profile)

	if route.size() > 0:
		return route

	return _checkout_route_for_score(score, max_darts)


func _exact_checkout_route_from_profile(score: int, max_darts: int, profile: CharacterResource = null) -> Array[String]:
	...
	var routes: Dictionary = profile.preferred_checkout_routes
	var route_value: Variant = null

	if routes.has(score):
		route_value = routes[score]
	elif routes.has(str(score)):
		route_value = routes[str(score)]
	...
	elif route_value is PackedStringArray:
		for value in route_value:
			route.append(str(value))
	else:
		return no_route

	if route.size() <= 0 or route.size() > max_darts:
		return no_route

	if not _is_valid_checkout_route(score, route):
		return no_route

	return route


func _is_valid_checkout_route(score: int, route: Array[String]) -> bool:
	var route_total: int = 0

	for label in route:
		route_total += _score_for_label(label)

	if route_total != score:
		return false

	return _is_finish_label(route[route.size() - 1])


func _is_finish_label(label: String) -> bool:
	return label == "BULL" or (label.length() >= 2 and label.substr(0, 1) == "D")


func _should_ai_attack_maximum(thrower: int = THROWER_AI) -> bool:
	if _score_for_thrower(thrower) <= 230:
		return false

	var profile: CharacterResource = _profile_for_thrower(thrower)
	var is_opening_phase: bool = _leg_darts_for_thrower(thrower) < 9
	var appetite: float = clampf(profile.maximum_appetite, 0.0, 2.0)

	if profile.average_max >= 100.0:
		return randf() < (0.38 if is_opening_phase else 0.24) * appetite

	if profile.average_max >= 90.0:
		return randf() < (0.16 if is_opening_phase else 0.08) * appetite

	return false


func _ai_setup_target_for_checkout(thrower: int = THROWER_AI) -> String:
	var profile: CharacterResource = _profile_for_thrower(thrower)
	var score: int = _score_for_thrower(thrower)
	var preferred_doubles: Array[String] = profile.preferred_checkout_doubles
	...
	for preferred_double in preferred_doubles:
		...
		var required_score: int = score - leave_score
	...
	for setup_label in ROUTE_DART_LABELS:
		var remaining_score: int = score - _score_for_label(setup_label)
...
func _ai_landed_label_for_target(target_label: String, thrower: int = THROWER_AI) -> String:
	var hit_chance: float = _ai_hit_chance_for_target(target_label, thrower)
...
func _ai_hit_chance_for_target(target_label: String, thrower: int = THROWER_AI) -> float:
	var profile: CharacterResource = _profile_for_thrower(thrower)
	...
	if multiplier_label == "D" or target_label == "BULL":
		chance -= _ai_pressure_level(thrower) * (1.0 - profile.nerve) * 0.28
	else:
		chance -= _ai_pressure_level(thrower) * (1.0 - profile.nerve) * 0.08
...
func _ai_pressure_level(thrower: int = THROWER_AI) -> float:
	var own_score: int = _score_for_thrower(thrower)
	var opponent_score: int = _player_score if thrower == THROWER_AI else _ai_score
	var ai_close: bool = own_score <= 80
	var player_close: bool = opponent_score <= 80
...
func _profile() -> CharacterResource:
	if _ai_profile == null:
		_ai_profile = CharacterResource.new()

	return _ai_profile


func _profile_for_thrower(thrower: int) -> CharacterResource:
	if thrower == THROWER_PLAYER:
		if _player_profile == null:
			_player_profile = CharacterResource.new()
...
func _leg_darts_for_thrower(thrower: int) -> int:
	return _player_leg_darts if thrower == THROWER_PLAYER else _ai_leg_darts
...
func _overlay_lock_position(offset: Vector2) -> Vector2:
	return _board_local_to_overlay(offset)


func _aim_radius() -> float:
	var board_center: Vector2 = _board_local_to_overlay(Vector2.ZERO)
	var board_edge: Vector2 = _board_local_to_overlay(Vector2(AIM_RADIUS, 0.0))

	return maxf(1.0, board_center.distance_to(board_edge))


func _board_visual_scale() -> float:
	return maxf(0.1, _aim_radius() / AIM_RADIUS)
...
func _draw_aim_guides() -> void:
	...
	if _phase == THROW_PHASE_LOCK_HEIGHT:
		...
		_last_drawn_phase = THROW_PHASE_LOCK_HEIGHT
		_last_drawn_y = current_y
		var half_width: float = _safe_axis_limit(current_y)
		var start: Vector2 = _board_local_to_overlay(Vector2(-half_width, current_y))
		var end: Vector2 = _board_local_to_overlay(Vector2(half_width, current_y))

		draw_line(start, end, AIM_LINE_COLOR, 3.0, true)
		_draw_edge_marker(start, Vector2(-1.0, 0.0))
		_draw_edge_marker(end, Vector2(1.0, 0.0))
	elif _phase == THROW_PHASE_LOCK_WIDTH:
		var current_x: float = _get_current_x()
		_last_drawn_phase = THROW_PHASE_LOCK_WIDTH
		_last_drawn_x = current_x
		var half_height: float = _safe_axis_limit(current_x)
		var locked_half_width: float = _safe_axis_limit(_locked_y)
		var locked_start: Vector2 = _board_local_to_overlay(Vector2(-locked_half_width, _locked_y))
		var locked_end: Vector2 = _board_local_to_overlay(Vector2(locked_half_width, _locked_y))
		var vertical_start: Vector2 = _board_local_to_overlay(Vector2(current_x, -half_height))
		var vertical_end: Vector2 = _board_local_to_overlay(Vector2(current_x, half_height))
		var crosshair_position: Vector2 = _board_local_to_overlay(Vector2(current_x, _locked_y))

		draw_line(locked_start, locked_end, LOCKED_LINE_COLOR, 2.0, true)
		draw_line(vertical_start, vertical_end, AIM_LINE_COLOR, 3.0, true)
		_draw_crosshair(crosshair_position, AIM_LINE_COLOR)
...
func _visible_lock_y() -> float:
	if _last_drawn_phase == THROW_PHASE_LOCK_HEIGHT:
		return _last_drawn_y

	return _get_current_y()


func _visible_lock_x() -> float:
	if _last_drawn_phase == THROW_PHASE_LOCK_WIDTH:
		return _last_drawn_x

	return _get_current_x()
...
func _board_local_to_overlay(local_position: Vector2) -> Vector2:
	...
	if dartboard == null:
		return BOARD_CENTER_FALLBACK + local_position * maxf(0.1, board_scale)

	return to_local(dartboard.to_global(local_position))


func _overlay_position_to_board_local(overlay_position: Vector2) -> Vector2:
	var dartboard: Node2D = get_node_or_null(dartboard_path) as Node2D

	if dartboard == null:
		return (overlay_position - BOARD_CENTER_FALLBACK) / maxf(0.1, board_scale)

	return dartboard.to_local(to_global(overlay_position))
